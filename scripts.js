// 1. Create an array of fruits and use a loop to log each fruit in the console.
{
...
// a paragraph with the ID output using document.querySelector.
{
    ...
    if (output) {
        for (let i = 0; i < colors.length; i++) {
            output.textContent += colors[i] + (i < colors.length - 1 ? " " : "");
        }
    }
}
...
// with ID names to show all names separated by commas.
{
    ...
    const output = document.querySelector("#names");
    if (output) {
        let text = "";
...
// largest number in a paragraph with ID max.
{
    ...
    const maxPara = document.querySelector("#max");
    if (maxPara) {
        maxPara.textContent = "Largest number: " + max;
    }
}
...
// 11. Use document.querySelector to select a div with ID result and change its text to "Task Complete".
{
    ...
    if (resultDiv) {
        resultDiv.textContent = "Task Complete";
    }
}
...
// display it inside an element with ID averageTemp.
{
    ...
    const avgPara = document.querySelector("#averageTemp");
    if (avgPara) {
        avgPara.textContent = "Average Temperature: " + average;
    }
}
...
// paragraph with ID totalSum.
{
    ...
    const totalSumPara = document.querySelector("#totalSum");
    if (totalSumPara) {
        totalSumPara.textContent = "Total Sum: " + total;
    }
}
...
// ID animals.
{
    ...
    const animalsPara = document.querySelector("#animals");
    if (animalsPara) {
        animalsPara.textContent = text;
    }
}
...
// "Welcome to JavaScript!".
{
    ...
    if (titleH1) {
        titleH1.textContent = "Welcome to JavaScript!";
    }
}
...
// display the count in a div with ID highScores.
{
    ...
    const scoresDiv = document.querySelector("#highScores");
    if (scoresDiv) {
        scoresDiv.textContent = "Scores above 75: " + count;
    }
}
...
// document.querySelector.
{
    ...
    if (messagePara) {
        messagePara.textContent = "Updated successfully";
    }
}
...
// them in a paragraph with ID subjectsList.
{
    ...
    const subjectsPara = document.querySelector("#subjectsList");
    if (subjectsPara) {
        subjectsPara.textContent = text;
    }
}

// 20. Write a loop that prints numbers from 10 down to 1 and shows the result inside #countdown.
// Also calculate sum of array [5, 10, 15, 20] and print to #sumResult.
{
    let text = "";
    for (let i = 10; i >= 1; i--) {
        text += i + " ";
    ...
    if (countdownPara) {
        countdownPara.textContent = "Countdown: " + text;
    }
    ...
    const sumResultPara = document.querySelector("#sumResult");
    if (sumResultPara) {
        sumResultPara.textContent = "Sum of array: " + total;
    }
}
...
// 30. Given an array [1, 2, 3, 4, 5], reverse the array manually using a loop (don’t use .reverse()).
{
